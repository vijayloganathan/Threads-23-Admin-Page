...
let day6 = date6.getDate();
let month6 = date6.getMonth() + 1;
let year6 = date6.getFullYear();
let hour6 = date6.getHours();
let min6 = date6.getMinutes();
let sec6 = date6.getSeconds();
let currentDate6 = `${day6}-${month6}-${year6}  ${hour6}-${min6}-${sec6}`;
...
{ 
  ...
  let i=0;
  let codigoid=[];
  databas.ref("userdata").child("event6").on("value", function(snapshot) {
    snapshot.forEach(function(childSnapshot) {
      ...
      var table = document.getElementById("event6_table");
      let a=1;
      databas.ref("userdata").child("user").once("value", function(snapshot) {
        table.querySelector("#event6_tbody").innerHTML = "";
        let rownum=0;
        snapshot.forEach(function(childSnapshot) {
          rownum=rownum+1;
          var data = childSnapshot.val();
          ...
          cell5.addEventListener("click", function() {
             ...
             databas.ref("userdata").child("user").child(id).get().then((snapshot)=>{
              if (snapshot.exists()) {
                ...
                database.ref("userdata").child("event6").child(id).set({
                  id : id,
                  Name : name,
                  Phoneno : phone,
                  college : clg ,
...
const user_event6btn = document.getElementById("user_event6btn");
user_event6btn.addEventListener('click' , event => {
  event.preventDefault()
        // Declare variables
        var input, filter, table, tr, td, i, txtValue;
        input = document.getElementById("user_event6");
        filter = input.value.toUpperCase();
        table = document.getElementById("event6_table");
    ...
    const report_event6btn = document.getElementById("report_event6btn");
report_event6btn.addEventListener('click' , event => {

  var ref = firebase.database().ref("userdata").child("event6");
  ref.once('value', function(snapshot) {
    ...
    XLSX.writeFile(workbook, 'QUIZ HOLIC report on '+currentDate6+'.xlsx');
});
