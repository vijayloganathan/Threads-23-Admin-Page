...
let day8 = date8.getDate();
let month8 = date8.getMonth() + 1;
let year8 = date8.getFullYear();
let hour8 = date8.getHours();
let min8 = date8.getMinutes();
let sec8 = date8.getSeconds();
let currentDate8 = `${day8}-${month8}-${year8}  ${hour8}-${min8}-${sec8}`;
...
{ 
  ...
  let i=0;
  let codigoid=[];
  databas.ref("userdata").child("event8").on("value", function(snapshot) {
    snapshot.forEach(function(childSnapshot) {
      ...
      var table = document.getElementById("event8_table");
      let a=1;
      databas.ref("userdata").child("user").once("value", function(snapshot) {
        table.querySelector("#event8_tbody").innerHTML = "";
        let rownum=0;
        snapshot.forEach(function(childSnapshot) {
          rownum=rownum+1;
          var data = childSnapshot.val();
          ...
          cell5.addEventListener("click", function() {
             ...
             databas.ref("userdata").child("user").child(id).get().then((snapshot)=>{
              if (snapshot.exists()) {
                ...
                database.ref("userdata").child("event8").child(id).set({
                  id : id,
                  Name : name,
                  Phoneno : phone,
                  college : clg ,
...
const user_event8btn = document.getElementById("user_event8btn");
user_event8btn.addEventListener('click' , event => {
  event.preventDefault()
        // Declare variables
        var input, filter, table, tr, td, i, txtValue;
        input = document.getElementById("user_event8");
        filter = input.value.toUpperCase();
        table = document.getElementById("event8_table");
    ...
    const report_event8btn = document.getElementById("report_event8btn");
report_event8btn.addEventListener('click' , event => {

  var ref = firebase.database().ref("userdata").child("event8");
  ref.once('value', function(snapshot) {
    ...
    XLSX.writeFile(workbook,'TREASURE HUNT report on '+currentDate8+'.xlsx');
});
